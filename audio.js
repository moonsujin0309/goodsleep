...
export class Mixer {
   ...
   *  아주 작았기 때문이다(발화 RMS 중앙값 0.055, 파일 사이 편차 16.5배). 배경음은 RMS 0.108
   *  이라 같은 볼륨이면 목소리가 배경에 묻혔고, 광대역 소음은 속삭임을 RMS 비율보다 훨씬
   *  심하게 덮는다 — 말은 짧은 봉우리에 에너지가 몰려 있고 소음은 끊기지 않기 때문이다.
   *  그래서 0.22 로도 "목소리가 안 들린다"가 두 번 나왔다.
   *
   *  이제 나레이션을 굽는 단계에서 발화 RMS 0.11 로 맞춘다(tts_voxcpm.NORM_RMS).
   *  목소리가 3배 커진 덕에 **배경음을 누를 이유가 없어졌다** — DUCK 참조. */

  /** 나레이션 중 배경음을 얼마나 누를지. 1 이면 안 누른다 (2026-08-26 부터 기본).
   *
   *  순서가 거꾸로였다. 배경음 크기는 사용자가 준비 화면에서 정하는 값인데,
   *  그걸 앱이 다시 눌러 버리면 사용자가 정한 게 의미가 없어진다.
   *  맞춰야 하는 쪽은 목소리다 — 그래서 나레이션을 굽는 단계에서 키웠다(NORM_RMS 0.11).
   *
   *  지금 균형: 배경음은 소스 RMS 0.108 에 이 배수가 곱해진다. 배경음을 최대로 올려도
   *  (레이어 1.0 · master 1.0) 실효 RMS 0.0162 이고 목소리는 0.11 이라 6.8배 위에 있다.
   *  기본 프리셋(파도 0.6)이면 19배다.
   *
   *  그래도 광대역 소음은 속삭임을 RMS 비율보다 심하게 덮는다. 목소리가 묻히면
   *  0.6 → 0.4 순으로 내려 본다. 0.08 은 배경음이 아예 안 들린다는 증상을 낳았던 값이다. */
  static DUCK = 1;
