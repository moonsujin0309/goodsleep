...
export class Mixer {
   ...
   *  목소리가 1.9배 커졌으니 배경도 그만큼 올릴 수 있다. 둘은 같이 움직여야 한다 —
   *  NORM_RMS 를 바꾸면 이 값도 함께 봐야 균형이 유지된다.
   *  "나레이션 중에 배경음이 아예 안 들린다"가 0.08 의 증상이었다. */
  _target(layer) {
    return layer.volume * layer.volume * 0.15 * this.master * (this.ducked ? 0.20 : 1);
  }
