...
MP3_KBPS = 64
# 굽기 직전에 시간축으로 늘리는 배수. **지금은 1.0 = 끔이다** (2026-08-26).
#
# 한때 1.12 였다. 모델이 명상에 쓰기엔 빠르게 말해서(초당 6.9음절) 뽑은 뒤에 늘렸다.
# 그런데 늘리기에는 천장이 있다 — 1.45 배를 넘으면 자음이 뭉개진다.
#
# 지금은 **앵커를 늦춰서** 모델이 처음부터 느리게 말하게 한다. 앵커를 1.43배로
# 늘려 두면 결과가 1.4배 가까이 느려진다 (실측: 앵커 1.25배 → 결과 1.27배).
# 가공이 아니라 생성이라 음질 손해가 없고 천장도 없다. 더 느리게 하려면
# 이 값이 아니라 tools/anchor/voice.wav 를 늘린다.
#
# 그래도 켜야 한다면 tempo() = ffmpeg atempo 로만 한다 — librosa 는 쇳소리가 난다.
SLOW = 1.0
